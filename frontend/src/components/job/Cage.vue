...
function getGardent() {
    ...
    axios.get(`${store.api}/api/cage`, store.header)
        .then(response => {
...
function getGardentSlot() {
    ...
    axios.get(`${store.api}/api/cage/slot`, store.header)
        .then(response => {
...
function deleteGardent() {
    ...
    axios.delete(`${store.api}/api/cage`, store.header)
        .then(response => {
            gardent.value = null
            store.noti = {
                'title': 'success',
...
function buyGardent() {
    ...
    axios.post(`${store.api}/api/cage`, form, store.header)
        .then(response => {
            getGardent()
            getGardentSlot()
            store.loading = false
...
function upgrateGardent() {
    ...
    axios.get(`${store.api}/api/cage/upgrate`, store.header)
        .then(response => {
            getGardent()
            store.noti = {
                'title': 'success',
...
function buySlot() {
    ...
    axios.post(`${store.api}/api/cage/slot`, {}, store.header)
        .then(response => {
            getGardentSlot()
            store.noti = {
                'title': 'success',
                'content': 'Mở rộng thành công'
            }
            store.loading = false
        })
        .catch(error => {
...
<template>
    <div class="align-items-center d-flex flex-column gap-4" v-if="gardent">
        ...
        <div class="button-group">
            <button type="button" class="btn btn-success" @click="buySlot">Mở rộng</button>
