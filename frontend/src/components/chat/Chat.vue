...
function getChat(type) {
    ...
    axios.get(`${store.api}/api/chat?type=${type}`, store.header)
        .then(response => {
            chat.value = response.data.results
            store.loading = false
        })
        .catch(error => {
            store.noti = {
    ...
    axios.get(path, store.header)
        .then(response => {
            for (let i = 0; i < response.data.results.length; i++) {
                chat.value.results.push(response.data.results[i])
            }
            chat.value.next = response.data.next
            store.loading = false
        })
        .catch(error => {
            store.noti = {
...
function reload() {
    getChat(active.value)
}

</script>

<template>
    <div class="bag" v-if="chat">
        <div class="bag-tab">
        ...
        <div class="bag-content">
            <div class="item-content-row" v-for="i in chat">
                <div class="item-row-left">
                    <b>{{ i.sender.name }}</b>
                </div>
                <div class="item-row-right">
                    <p>{{ formatDate(i.created_at) }}</p>
                    <div class="icon-normal" @click="addCard('chat_detail', {
                        name: i.sender.name,
                        path: i.id,
                        tab: active
                    })">
            ...
            <div v-if="chat.next" class="w-100 text-center mt-2">
                <button type="button" class="btn btn-primary" @click="loadMore(chat.next)">Xem thêm</button>
            </div>
        </div>
    </div>
    <div v-else></div>
